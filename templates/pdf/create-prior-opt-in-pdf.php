<?php
	...
	$pdf->SetTitle('Prior Opt In summary');
	$pdf->SetSubject('Prior Opt In summary');
	$pdf->SetKeywords('Prior Opt In summary');
	...
	$pdf->writeHTMLCell(150, '', '', '', "Prior Opt In Summary", 0, 1, 1, true, 'C', true);
	...
	$pdf->writeHTMLCell(150, '', '', '', '<i>'.date("jS M Y",strtotime('tomorrow')).'</i>', 0, 1, 1, true, 'C', true);
	...
	$optInInfo = $wpdb->get_results( "SELECT * FROM $tablename where date=$tomorrowTime ");
	...
	$PdfName = CUS_WOO_ORDER_PLUGIN_DIR.'/pdffile/prior_opt_in_'.$stateCode.'.pdf';
